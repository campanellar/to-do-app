 ...
 * @author stuetzlec
 */
public class Gipf_Game {

    private Gipf_Playable[] players;
...
// Values for the possible states of each intersection
     ...
     */
    public Gipf_Game(int seed) {
        // Initialize the various fields
        this.rng = new Random(seed);
        players = new Gipf_Playable[2];
     ...
     */
    public int playGame(Gipf_Playable p1, Gipf_Playable p2) {
    ...
    private boolean push(Integer piece, Integer col, Integer pos, Integer dir)
        ...
        try {
            ...
            //  return true
            if (board[col][pos].equals(Gipf_Game.EMPTY)) {
                board[col][pos] = piece;
                return true;
            }
    ...
    public void checkForFourInARow() {
        ...
        for (Run r : runs) {
            ...
            while (this.isInRange(col, pos)
                    && !board[col][pos].equals(Gipf_Game.EMPTY)
    ...
    public Integer convertLetterToIndex(String letter) {
    ...
    private boolean isSamePlayer(Integer piece, Integer curRun) {
        return (piece % 2 == curRun % 2) && !piece.equals(Gipf_Game.EMPTY);
    }
        ...
        for (int col = 0; col < 9; col++) {
            for (int pos = 0; pos < board[col].length; pos++) {
                System.out.printf("%12s", Character.toString(letterValues[col])
                        ...
                        + (board[col][pos].equals(Gipf_Game.EMPTY) ? " " : board[col][pos]));
            }
            System.out.println();
        }
    }
